...
<CapEmbed src="https://cap.so/embed/k310v2vf41d6ffp" />
...
Create a Google Cloud OAuth client to enable Google sign-in and Google Drive storage:
...
4. Add this authorized redirect URI:
   ```
   https://your-cap-domain.example/api/desktop/storage/google-drive/callback
   ```
5. Add your Cap domain as an authorized JavaScript origin if you also use Google sign-in.
...
Cap requests the `drive.file` scope and creates a private `Cap` folder. It can
only access files created or opened through Cap. `WEB_URL` and `NEXTAUTH_URL`
must match the public HTTPS origin used in the redirect URI.
